...
namespace argparse {
  ...
  template <typename T>
  struct has_ostream_operator<T, decltype(void(std::declval<std::ostream&>() << std::declval<T const&>()))>
      : std::true_type {};

  inline std::string bold(std::string const& input_str) {
#ifdef _WIN32
...
#  pragma warning(pop)
      ...
      for (auto const& [name, value] : magic_enum::enum_entries<T>()) {
        if (v == name) {
  ...
  std::vector<std::string> inline split(std::string const& str) {
    std::vector<std::string> splits;
    std::stringstream ss(str);
    std::string key;
    while (std::getline(ss, key, ',')) {
      if (!key.empty() && key.back() == '\0')
        key.pop_back(); // last variables contain a '\0', which is unexpected when comparing to raw string, e.g. value
                        // == "test" will fail when the last character is '\0'. Therefore we can remove it
  ...
  template <typename T>
  std::string
  to_lower(T const& str_) { // both std::string and std::basic_string_view<char> (for magic_enum) are using to_lower
  ...
  template <typename T>
  inline T get(std::string const& v);
  template <>
  inline std::string get(std::string const& v) {
    return v;
  }
  template <>
  inline std::wstring get(std::string const& v) {
    return std::wstring(v.begin(), v.end());
  }
  template <>
  inline char get(std::string const& v) {
  ...
  template <>
  inline int get(std::string const& v) {
    return std::stoi(v);
  }
  template <>
  inline short get(std::string const& v) {
    return std::stoi(v);
  }
  template <>
  inline long get(std::string const& v) {
    return std::stol(v);
  }
  template <>
  inline long long get(std::string const& v) {
    return std::stol(v);
  }
  template <>
  inline bool get(std::string const& v) {
    return to_lower(v) == "true" || v == "1";
  }
  template <>
  inline float get(std::string const& v) {
    return std::stof(v);
  }
  template <>
  inline double get(std::string const& v) {
    return std::stod(v);
  }
  template <>
  inline unsigned char get(std::string const& v) {
    return get<char>(v);
  }
  template <>
  inline unsigned int get(std::string const& v) {
    return std::stoul(v);
  }
  template <>
  inline unsigned short get(std::string const& v) {
    return std::stoul(v);
  }
  template <>
  inline unsigned long get(std::string const& v) {
    return std::stoul(v);
  }
  template <>
  inline unsigned long long get(std::string const& v) {
    return std::stoul(v);
  }

  template <typename T>
  inline T get(std::string const& v) { // remaining types
    if constexpr (is_vector<T>::value) {
      std::vector<std::string> const splitted = split(v);
      T res(splitted.size());
      if (!v.empty())
      ...
      std::string const lower_str = to_lower(v);
      for (auto const& [value, name] : enum_entries) {
  ...
  struct ConvertBase {
    ...
    virtual void convert(std::string const& v) = 0;
    virtual void set_default(std::unique_ptr<ConvertBase> const& default_value, std::string const& default_string) = 0;
  ...
  struct ConvertType : public ConvertBase {
    ...
    explicit ConvertType(T const& value) : ConvertBase(), data(value) {};

    void convert(std::string const& v) override {
      data = get<T>(v);
    }

    void set_default(std::unique_ptr<ConvertBase> const& default_value, std::string const& default_string) override {
      if (this->get_type_id() ==
          default_value->get_type_id()) // When the types do not match exactly. resort to string conversion
  ...
  struct Entry {
    ...
    Entry(ARG_TYPE type,
        ...
        : type(type), keys_(split(key)), help(std::move(help)), implicit_value_(std::move(implicit_value)) {
    }

    // Allow both string inputs and direct-type inputs. Where a string-input will be converted like it would when using
    // the commandline, and the direct approach is to simply use the value provided.
    template <typename T>
    Entry& set_default(T const& default_value) {
    ...
    template <typename T>
    operator T() {
    } // When you get here  because you received an error, make sure all parameters of argparse are references (e.g.
      // with `&`)
  ...
  private:
    ...
    void _convert(std::string const& value) {
      try {
        this->value_ = value;
        datap->convert(value);
    ...
    [[nodiscard]] std::string info() const {
      std::string const allowed_entries = datap->get_allowed_entries();
      std::string const default_value = default_str_.has_value() ? "default: " + *default_str_ : "required";
      std::string const implicit_value = implicit_value_.has_value() ? "implicit: \"" + *implicit_value_ + "\", " : "";
      std::string const allowed_value =
  ...
  private:
    ...
    bool has_options() {
      return std::find_if(all_entries.begin(), all_entries.end(), [](auto e) { return e->type != Entry::ARG; }) !=
             all_entries.end();
    };
  ...
  public:
     ...
     */
    Entry& arg(std::string const& help) {
      return arg("arg_" + std::to_string(_arg_idx), help);
    }

    /* Add a *named* positional argument, the order in which it is defined equals the order in which they are being
     * read. key : The name of the argument, otherwise arg_<position> will be used help : Description of the variable
     *
     * Returns a reference to the Entry, which will collapse into the requested type in `Entry::operator T()`
     */
    Entry& arg(std::string const& key, std::string const& help) {
      std::shared_ptr<Entry> entry = std::make_shared<Entry>(Entry::ARG, key, help);
      // Increasing _arg_idx, so that arg2 will be arg_2, irregardless of whether it is preceded by other positional
      // arguments
      _arg_idx++;
     ...
     * Returns a reference to the Entry, which will collapse into the requested type in `Entry::operator T()`
     */
    Entry& kwarg(std::string const& key,
                 std::string const& help,
                 std::optional<std::string> const& implicit_value = std::nullopt) {
      std::shared_ptr<Entry> entry = std::make_shared<Entry>(Entry::KWARG, key, help, implicit_value);
      all_entries.emplace_back(entry);
      for (std::string const& k : entry->keys_) {
     ...
     */
    Entry& flag(std::string const& key, std::string const& help) {
     ...
     */
    SubcommandEntry& subcommand(std::string const& command) {
    ...
    virtual void help() {
      ...
      for (auto const& entry : arg_entries)
        cout << entry->keys_[0] << ' ';
      if (has_options())
        cout << " [options...]";
      if (!subcommand_entries.empty()) {
        cout << " [SUBCOMMAND: ";
        for (auto const& [subcommand, subentry] : subcommand_entries) {
          cout << subcommand << ", ";
        }
        cout << "]";
      }
    ...
    void validate(bool const& raise_on_error) {
      for (auto const& entry : all_entries) {
     ...
     */
    void parse(int argc, char const* const* argv, bool const& raise_on_error) {
      ...
      argc = parse_subcommands(); // argc_ is the number of arguments that should be parsed after the subcommand has
                                  // finished parsing
      ...
      auto is_value = [&](size_t const& i) -> bool {
        return params.size() > i &&
               (params[i][0] != '-' ||
                (params[i].size() > 1 &&
                 std::isdigit(
                     params[i][1]))); // check for number to not accidentally mark negative numbers as non-parameter
      };
      auto parse_param = [&](size_t& i,
                             std::string const& key,
                             bool const is_short,
                             std::optional<std::string> const& equal_value = std::nullopt) {
        ...
        if (itt != kwarg_entries.end()) {
          ...
          if (equal_value.has_value()) {
            ...
            if (is_value(i + 1)) {
              ...
              entry->_convert(
                  ""); // for multiargument parameters, return an empty vector when not passing any more values
            } else {
      ...
      auto add_param = [&](size_t& i, size_t const& start) {
      ...
      for (size_t i = 0; i < params.size(); i++) {
        if (!is_value(i)) {
          if (params[i].size() > 1 && params[i][1] == '-') { // long --
            ...
            size_t const j_end = std::min(params[i].size(), params[i].find('=')) - 1;
            for (size_t j = 1; j < j_end; j++) { // add possible other flags
              std::string const key = std::string(1, params[i][j]);
      ...
      // Parse all the positional arguments, making sure multi_argument positional arguments are processed last to
      ...
      for (size_t j_end = arg_entries.size() - arg_i; arg_j <= j_end;
           arg_j++) { // iterate from back to front, to ensure non-multi-arguments in the front and back are given
                      // preference
  ...
  template <typename T>
  T parse(int argc, char const* const* argv, bool const& raise_on_error = false) {
    T args = T();
