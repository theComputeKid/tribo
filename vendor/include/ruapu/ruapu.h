...
int ruapu_supports(char const* isa);

char const* const* ruapu_rua();
...
static int ruapu_detect_isa(ruapu_some_inst some_inst) {
  int volatile ruapu_sigill_caught = 0;

  __try {
    some_inst();
  } __except (_exception_code() == RUAPU_STATUS_ILLEGAL_INSTRUCTION ? RUAPU_EXCEPTION_EXECUTE_HANDLER
                                                                    : RUAPU_EXCEPTION_CONTINUE_SEARCH) {
...
#  endif // defined _WIN32 || defined __ANDROID__ || defined __linux__ || defined __APPLE__ || defined __FreeBSD__ ||
         // defined __NetBSD__ || defined __OpenBSD__ || defined __DragonFly__ || defined __sun__ || defined
         // __SYTERKIT__
...
#          define RUAPU_INSTCODE(isa, ...)                                                                             \
            __pragma(section(".text"))                                                                                 \
                __declspec(allocate(".text")) static unsigned short ruapu_some_##isa[] = {__VA_ARGS__, 0x4770};
#        else
#          define RUAPU_INSTCODE(isa, ...)                                                                             \
            __attribute__((section(".text"))) static unsigned short ruapu_some_##isa[] = {__VA_ARGS__, 0x4770};
#        endif
#      else
#        ifdef _MSC_VER
#          define RUAPU_INSTCODE(isa, ...)                                                                             \
...
#        define RUAPU_INSTCODE(isa, ...)                                                                               \
          static void ruapu_some_##isa() {                                                                             \
            asm volatile(".short " #__VA_ARGS__);                                                                      \
          }
#      else
#        define RUAPU_INSTCODE(isa, ...)                                                                               \
          static void ruapu_some_##isa() {                                                                             \
...
// csrr res, vlenb
#    define RUAPU_DETECT_ZVL(len)                                                                                      \
      static void ruapu_some_zvl##len##b() {                                                                           \
        RUAPU_RVV1P0_AVAIL();                                                                                          \
        intptr_t res;                                                                                                  \
...
#    define RUAPU_RVV_INSTCODE(isa, vtype, ...)                                                                        \
      static void ruapu_some_##isa() {                                                                                 \
        RUAPU_RVV1P0_AVAIL();                                                                                          \
        intptr_t res;                                                                                                  \
...
#  if defined(__i386__) || defined(__x86_64__) || defined(_M_IX86) || defined(_M_X64)
    RUAPU_ISAENTRY(mmx) RUAPU_ISAENTRY(sse) RUAPU_ISAENTRY(sse2) RUAPU_ISAENTRY(sse3) RUAPU_ISAENTRY(ssse3)
        RUAPU_ISAENTRY(sse41) RUAPU_ISAENTRY(sse42) RUAPU_ISAENTRY(sse4a) RUAPU_ISAENTRY(xop) RUAPU_ISAENTRY(avx)
            RUAPU_ISAENTRY(f16c) RUAPU_ISAENTRY(fma) RUAPU_ISAENTRY(fma4) RUAPU_ISAENTRY(avx2) RUAPU_ISAENTRY(avx512f)
                RUAPU_ISAENTRY(avx512bw) RUAPU_ISAENTRY(avx512cd) RUAPU_ISAENTRY(avx512dq) RUAPU_ISAENTRY(avx512vl)
                    RUAPU_ISAENTRY(avx512vnni) RUAPU_ISAENTRY(avx512bf16) RUAPU_ISAENTRY(avx512ifma)
                        RUAPU_ISAENTRY(avx512vbmi) RUAPU_ISAENTRY(avx512vbmi2) RUAPU_ISAENTRY(avx512fp16)
    // TODO:avx512pf
    RUAPU_ISAENTRY(avx512er) RUAPU_ISAENTRY(avx5124fmaps) RUAPU_ISAENTRY(avx5124vnniw) RUAPU_ISAENTRY(avxvnni)
        RUAPU_ISAENTRY(avxvnniint8) RUAPU_ISAENTRY(avxvnniint16) RUAPU_ISAENTRY(avxifma) RUAPU_ISAENTRY(avxneconvert)
            RUAPU_ISAENTRY(amxfp16) RUAPU_ISAENTRY(amxbf16) RUAPU_ISAENTRY(amxint8) RUAPU_ISAENTRY(amxtile)
                RUAPU_ISAENTRY(bmi1) RUAPU_ISAENTRY(bmi2) RUAPU_ISAENTRY(gfni) RUAPU_ISAENTRY(aesni)
                    RUAPU_ISAENTRY(vaes) RUAPU_ISAENTRY(sha1) RUAPU_ISAENTRY(sha256) RUAPU_ISAENTRY(sha512)
                        RUAPU_ISAENTRY(sm3) RUAPU_ISAENTRY(sm4) RUAPU_ISAENTRY(rdrand) RUAPU_ISAENTRY(rdseed)
                            RUAPU_ISAENTRY(tsx)

#  elif __aarch64__ || defined(_M_ARM64)
    RUAPU_ISAENTRY(neon) RUAPU_ISAENTRY(vfpv4) RUAPU_ISAENTRY(lse) RUAPU_ISAENTRY(cpuid) RUAPU_ISAENTRY(asimdrdm)
        RUAPU_ISAENTRY(asimdhp) RUAPU_ISAENTRY(asimddp) RUAPU_ISAENTRY(asimdfhm) RUAPU_ISAENTRY(bf16)
            RUAPU_ISAENTRY(i8mm) RUAPU_ISAENTRY(frint) RUAPU_ISAENTRY(jscvt) RUAPU_ISAENTRY(fcma) RUAPU_ISAENTRY(mte)
                RUAPU_ISAENTRY(mte2) RUAPU_ISAENTRY(sve) RUAPU_ISAENTRY(sve2) RUAPU_ISAENTRY(sve2p1)
                    RUAPU_ISAENTRY(svebf16) RUAPU_ISAENTRY(svei8mm) RUAPU_ISAENTRY(svef32mm) RUAPU_ISAENTRY(svef64mm)
                        RUAPU_ISAENTRY(sme) RUAPU_ISAENTRY(smef16f16) RUAPU_ISAENTRY(smef64f64)
                            RUAPU_ISAENTRY(smei64i64) RUAPU_ISAENTRY(pmull) RUAPU_ISAENTRY(crc32) RUAPU_ISAENTRY(aes)
                                RUAPU_ISAENTRY(sha1) RUAPU_ISAENTRY(sha2) RUAPU_ISAENTRY(sha3) RUAPU_ISAENTRY(sha512)
                                    RUAPU_ISAENTRY(sm3) RUAPU_ISAENTRY(sm4) RUAPU_ISAENTRY(svepmull)
                                        RUAPU_ISAENTRY(svebitperm) RUAPU_ISAENTRY(sveaes) RUAPU_ISAENTRY(svesha3)
                                            RUAPU_ISAENTRY(svesm4) RUAPU_ISAENTRY(amx) RUAPU_ISAENTRY(paca)
                                                RUAPU_ISAENTRY(pacg)

#  elif __arm__ || defined(_M_ARM)
    RUAPU_ISAENTRY(half) RUAPU_ISAENTRY(edsp) RUAPU_ISAENTRY(neon) RUAPU_ISAENTRY(vfpv4) RUAPU_ISAENTRY(idiv)

#  elif __mips__
    RUAPU_ISAENTRY(msa) RUAPU_ISAENTRY(mmi) RUAPU_ISAENTRY(sx) RUAPU_ISAENTRY(asx) RUAPU_ISAENTRY(msa2)
        RUAPU_ISAENTRY(crypto)

#  elif __ppc__ || __powerpc__
    RUAPU_ISAENTRY(altivec) RUAPU_ISAENTRY(vsx)
...
#  elif __riscv
    RUAPU_ISAENTRY(i) RUAPU_ISAENTRY(m) RUAPU_ISAENTRY(a) RUAPU_ISAENTRY(f) RUAPU_ISAENTRY(d) RUAPU_ISAENTRY(c)
        RUAPU_ISAENTRY(p) RUAPU_ISAENTRY(v) RUAPU_ISAENTRY(zba) RUAPU_ISAENTRY(zbb) RUAPU_ISAENTRY(zbc) RUAPU_ISAENTRY(
            zbpbo) RUAPU_ISAENTRY(zbs) RUAPU_ISAENTRY(zbkb) RUAPU_ISAENTRY(zbkc) RUAPU_ISAENTRY(zbkx)
            RUAPU_ISAENTRY(zcb) RUAPU_ISAENTRY(zfa) RUAPU_ISAENTRY(zfbfmin) RUAPU_ISAENTRY(zfh) RUAPU_ISAENTRY(
                zfhmin) RUAPU_ISAENTRY(zicbop) RUAPU_ISAENTRY(zicond) RUAPU_ISAENTRY(zicsr) RUAPU_ISAENTRY(zifencei)
                RUAPU_ISAENTRY(zihintpause) RUAPU_ISAENTRY(zmmul) RUAPU_ISAENTRY(zpn) RUAPU_ISAENTRY(zpsfoperand)
                    RUAPU_ISAENTRY(zvbb) RUAPU_ISAENTRY(zvbc) RUAPU_ISAENTRY(zvfh) RUAPU_ISAENTRY(zvfhmin)
                        RUAPU_ISAENTRY(zvfbfmin) RUAPU_ISAENTRY(zvfbfwma) RUAPU_ISAENTRY(zvkb) RUAPU_ISAENTRY(zvl32b)
                            RUAPU_ISAENTRY(zvl64b) RUAPU_ISAENTRY(zvl128b) RUAPU_ISAENTRY(zvl256b)
                                RUAPU_ISAENTRY(zvl512b) RUAPU_ISAENTRY(zvl1024b)

                                    RUAPU_ISAENTRY(xsfvfnrclipxfqf) RUAPU_ISAENTRY(xsfvfwmaccqqq) RUAPU_ISAENTRY(
                                        xsfvqmaccdod) RUAPU_ISAENTRY(xsfvqmaccqoq)

                                        RUAPU_ISAENTRY(xtheadba) RUAPU_ISAENTRY(xtheadbb) RUAPU_ISAENTRY(xtheadbs)
                                            RUAPU_ISAENTRY(xtheadcondmov) RUAPU_ISAENTRY(xtheadfmemidx) RUAPU_ISAENTRY(
                                                xtheadfmv) RUAPU_ISAENTRY(xtheadmac) RUAPU_ISAENTRY(xtheadmemidx)
                                                RUAPU_ISAENTRY(xtheadmempair) RUAPU_ISAENTRY(xtheadsync)
                                                    RUAPU_ISAENTRY(xtheadvector) RUAPU_ISAENTRY(xtheadvdot)

                                                        RUAPU_ISAENTRY(xsmtvmadoti8) RUAPU_ISAENTRY(xsmtvmadoti4)
                                                            RUAPU_ISAENTRY(xsmtvmadotni8) RUAPU_ISAENTRY(xsmtvfmadotf32)
...
#  if defined _WIN32 || defined __ANDROID__ || defined __linux__ || defined __APPLE__ || defined __FreeBSD__ ||        \
      defined __NetBSD__ || defined __OpenBSD__ || defined __DragonFly__ || defined __sun__ || defined __SYTERKIT__
  size_t j = 0;
  for (size_t i = 0; i < sizeof(g_ruapu_isa_map) / sizeof(g_ruapu_isa_map[0]); i++) {
...
int ruapu_supports(char const* isa) {
  char const* const* isa_supported = g_ruapu_isa_supported;
  while (*isa_supported) {
