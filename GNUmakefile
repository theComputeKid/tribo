...
SRC_ALL = $(SRC_APP) $(SRC_BASE) $(SRC_CUDA)
TMP_ALL = $(TMP_APP) $(TMP_BASE) $(TMP_CUDA)
OBJ_ALL = $(OBJ_APP) $(OBJ_BASE) $(OBJ_CUDA)
...
cc:
	...
	@for f in $(SRC_APP); do echo "{ \"file\": \"$$f\", \"command\": \"$(CC) -c $(INC_DIR) $(DLL_MACRO) $(CFLAGS) $$f\", \"directory\": \"$(CURDIR)\" }," >> $(COMPILE_COMMANDS_FILE); done
	@for f in $(SRC_BASE); do echo "{ \"file\": \"$$f\", \"command\": \"$(CC) -c $(INC_DIR) $(DLL_MACRO) $(CFLAGS) $$f\", \"directory\": \"$(CURDIR)\" }," >> $(COMPILE_COMMANDS_FILE); done
