...
run help version:
ifeq ($(OS),linux)
	LD_LIBRARY_PATH=$(BIN_DIR) $(OUT_cli) --$@ --verbosity=2 --input=examples/simple.ini --output=$(TMP_DIR)/output.txt
else ifeq ($(OS),windows)
	$(OUT_cli) --$@ --verbosity=2 --input=examples/simple.ini --output=$(TMP_DIR)/output.txt
else ifeq ($(OS),macos)
	DYLD_LIBRARY_PATH=$(BIN_DIR) $(OUT_cli) --$@ --verbosity=2 --input=examples/simple.ini --output=$(TMP_DIR)/output.txt
endif
