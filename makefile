...
NAME_UTILS = lib$(PRJ_NAME)-utils
SRC_UTILS = log.c
DIR_UTILS = src\utils
TMP_UTILS = $(TMP_DIR)\$(DIR_UTILS)
OBJ_UTILS = $(patsubst %.c,$(TMP_UTILS)\\%.obj,$(SRC_UTILS))
SRC_FULL_UTILS = $(patsubst %,$(DIR_UTILS)\\%,$(SRC_UTILS))
OUT_DIR_UTILS = /Fa$(TMP_UTILS)\ /Fo$(TMP_UTILS)\ /Fd$(TMP_UTILS)\\
...
# All source files listed by language.
SRC_C = $(SRC_FULL_APP) $(SRC_FULL_BASE) $(SRC_FULL_UTILS)

# Clang-tidy files.
SRC_TIDY = $(SRC_C)

# Determine which targets to build for "all".
...
# Rules for creating object files for the utils project.
{$(DIR_UTILS)}.c{$(TMP_UTILS)}.obj::
	@if not exist $(TMP_UTILS) mkdir $(TMP_UTILS)
	$(CC) /c $(INC_DIR) $(CFLAGS) $(OUT_DIR_UTILS) $<
...
cc:
	...
	@for %f in ($(SRC_C:\=\\)) do @echo { "file": "%f", "command": "$(CC:\=\\) $(INC_DIR:\=\\) $(CFLAGS) %f", "directory": "$(MAKEDIR:\=\\)" }, >> $(COMPILE_COMMANDS_FILE)
!IF "$(CUDA_SUPPORT)" == "1"
	@for %f in ($(SRC_FULL_CUDA:\=\\)) do @echo { "file": "%f", "command": "clang-cl $(CUTIDYFLAGS) %f", "directory": "$(MAKEDIR:\=\\)" }, >> $(COMPILE_COMMANDS_FILE)
!ENDIF
...
deps:
	...
	cl $(CLDEPSFLAGS) $(INC_DIR) $(SRC_FULL_UTILS) > $(DEPS_DIR)\utils.txt
	...
	Write-NMakeDependencies -InputFile '$(DEPS_DIR)\utils.txt' -OutputFile '$(DEPS_FILE)' -SourceDir '$(DIR_UTILS)' -TempDir '$(TMP_UTILS)'"
...
clang-doc:
	...
	@if exist $(DOCS_DIR) rmdir /s /q $(DOCS_DIR)
	@mkdir $(DOCS_DIR)
	$(CLANG_DOC) --output=$(DOCS_DIR) --project-name=$(PRJ_NAME) $(CLANG_DOC_ARGS) $(COMPILE_COMMANDS_FILE) --executor=all-TUs

doxygen:
	@$(MAKE) /nologo cc CC=clang-cl
	@if exist $(DOCS_FOLDER) rmdir /s /q $(DOCS_FOLDER)
	@$(DOXYGEN) Doxyfile

run: app
	set TRIBO_VERBOSE=$(TRIBO_VERBOSE)
