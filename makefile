...
!INCLUDE tools\detect-arch-nmake.mak
!INCLUDE tools\compilers\setup-nmake.mak
...
EXE_APP = $(BIN_DIR)\$(PRJ_NAME)-$(PRJ_SUFFIX).exe
SRC_APP = main.c
DIR_APP = src\app
...
SRC_ALL = $(SRC_FULL_APP) $(SRC_FULL_BASE)
...
{$(DIR_APP)}.c{$(TMP_APP)}.obj::
	...
	$(CC) /c $(INC_DIR) $(CFLAGS) /Fa$(TMP_APP)\ /Fo$(TMP_APP)\ /Fd$(TMP_APP)\ $<
...
{$(DIR_BASE)}.c{$(TMP_BASE)}.obj::
	...
	$(CC) /c $(INC_DIR) $(CFLAGS) /Fa$(TMP_BASE)\ /Fo$(TMP_BASE)\ /Fd$(TMP_BASE)\ $<
...
cc:
	...
	@for %f in ($(SRC_APP)) do @echo { "file": "$(DIR_APP:\=/)/%f", "command": "clang-cl $(INC_DIR:\=/) $(CTIDYFLAGS) $(DIR_APP:\=/)/%f", "directory": "$(MAKEDIR:\=/)" }, >> $(COMPILE_COMMANDS_FILE)
	@for %f in ($(SRC_BASE)) do @echo { "file": "$(DIR_BASE:\=/)/%f", "command": "clang-cl $(INC_DIR:\=/) $(CTIDYFLAGS) $(DIR_BASE:\=/)/%f", "directory": "$(MAKEDIR:\=/)" }, >> $(COMPILE_COMMANDS_FILE)
	@echo { "file": "tools/clang-tidy.c", "command": "clang-cl /W0 tools/clang-tidy.c", "directory": "$(MAKEDIR:\=/)" } >> $(COMPILE_COMMANDS_FILE)
	@echo ] >> $(COMPILE_COMMANDS_FILE)

tidy: cc
	$(CLANGTIDY_CALL) $(SRC_ALL:\=\\)

# Equivalent to -MMD. Parses includes.
deps:
	...
	". tools\Write-NMakeDependencies.ps1; \
