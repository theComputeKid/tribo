...
$(OUT_CLI): $(OBJ_CLI) $(OUT_UTILS) $(OUT_LOADER) $(BIN_DIR)
	$(CC) $(OBJ_CLI) $(LDFLAGS) $(IMPLIB_UTILS) $(IMPLIB_LOADER) /PDB:$(basename $@).pdb /OUT:$@
...
$(OUT_BASE): $(OBJ_BASE) $(OUT_UTILS) $(BIN_DIR)
	$(CC) $(OBJ_BASE) $(LDFLAGS) /DLL /NOIMPLIB /PDB:$(basename $@).pdb /OUT:$@
...
$(OUT_AVX): $(OBJ_AVX) $(OUT_UTILS) $(BIN_DIR)
	$(CC) $(OBJ_AVX) $(LDFLAGS) /DLL /NOIMPLIB /PDB:$(basename $@).pdb /OUT:$@
...
$(OUT_NEON): $(OBJ_NEON) $(OUT_UTILS) $(BIN_DIR)
	$(CC) $(OBJ_NEON) $(LDFLAGS) /DLL /NOIMPLIB /PDB:$(basename $@).pdb /OUT:$@
...
run bench help version:
	$(OUT_CLI) --$@ --verbosity=2 --input=examples\simple.ini --output=tmp\simple-out.txt

tidy:
	$(TIDY) $(TIDY_FLAGS) $(SRC_CLI) -- $(CC) $(INC_DIR:/=-) $(CFLAGS)
	$(TIDY) $(TIDY_FLAGS) $(SRC_UTILS) -- $(CC) $(INC_DIR:/=-) $(DLL_EXPORT_BACKEND:/=-) $(CFLAGS)
